...
export default function Product({item}) {
  ...
  const {counterWish, setCounterWish, addToWishlist,getWishList} = useContext(Cartcontext);
        ...
        <div className="product cursor-pointer p-2 rounded-2 fw-fs">
          <Link to={`/Product-details/${item._id}`}>
            <div className="details">
              <img className='w-100' src={item.imageCover} alt='freshCart products' />
              <span className='text-main'>{item.category.name}</span>
              <h5 className='fw-fs fs-6'>{item.title.split(' ').slice(0,2).join(' ')}</h5>
          ...
          <button  disabled={loadingBtnWish} onClick={()=>addProductToWishlist(item._id)} className="bg-transparent my-btn border-0 py-3 fs-6 w-100">
            {loadingBtnWish?<i className='fa fa-spinner fa-spin'></i>:<i className={`fa-solid fa-heart ms-2 ${myHeartColor}`}  ></i>
}
          </button>
          <button disabled={loadingBtn} onClick={()=>addProductToCart(item._id)} className="btn bg-main w-100 text-white">
